...
const { requireCustomer, attachCustomerIfPresent } = require("./customers");
...
// Works for guests (no Authorization header) and for logged-in customers,
// in which case the booking is linked to their account via attachCustomerIfPresent.
  ...
  const info = db
    .prepare(
      `INSERT INTO bookings
        (category_id, vendor_id, customer_id, customer_name, customer_email, event_date, event_time, details_json, amount_total)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      categoryId,
...
// GET /api/bookings/mine-as-customer — bookings placed by the logged-in customer
router.get("/mine-as-customer", requireCustomer, (req, res) => {
  const rows = db
    .prepare(BOOKING_JOIN + " WHERE b.customer_id = ? ORDER BY b.created_at DESC")
    .all(req.customerId);
  res.json(rows.map(serializeBooking));
});
